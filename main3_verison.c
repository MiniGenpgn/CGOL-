...
void reprint(void)
{
    ...
    for(j=0;j<size;j++)
    {
        for (int dy=0;dy<2;dy++) // prints row twice
        {
            for(i=0;i<size;i++)
            {
                for (int dx=0;dx<2;dx++) // prints column twice
                {
...
void change(void)
{
    for(j=0;j<size;j++)
    {
        ...
        {   
            // checking each cell
            // 0-8 based on how many are alive
            
            int neighbors = grid[j-1][i-1]+grid[j-1][i]+grid[j-1][i+1]+grid[j][i-1]+grid[j][i+1]+grid[j+1][i-1]+grid[j+1][i]+grid[j+1][i+1];;
...
void rng(void)
{
    int z=0, randomx, randomy;
    
    while(z<size*3) // loop until counter less than three times the size
    {
        randomy = rand()%size; // randomize y
        randomx = rand()%size; // randomize x
